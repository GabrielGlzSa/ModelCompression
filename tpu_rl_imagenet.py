...
def train_step_fc(dataset_inputs):
    ...
    optimizer.apply_gradients(zip(gradients, fc_agent.model.trainable_weights))
    return td_loss

@tf.function
def distributed_train_step_fc(dataset_inputs):
    per_replica_losses = strategy.run(train_step_fc, args=(dataset_inputs,))
    return strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses,
                         axis=None)

@tf.function
def distributed_train_step_conv(dataset_inputs):
    per_replica_losses = strategy.run(train_step_conv, args=(dataset_inputs,))
    return strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses,
                         axis=None)
...
with tqdm(total=rl_iterations,
    ...
    for i in range(rl_iterations):
        ...
        agent = conv_agent
        target_network = conv_target_network
        optimizer = optimizer_conv
        n_actions = conv_n_actions
        conv_loss_t = distributed_train_step_conv(batch_data)
        td_loss_history_conv.append(conv_loss_t)

        # train
        batch_data = sample_batch(fc_exp_replay, batch_size=rl_batch_size)
        agent = fc_agent
        target_network = fc_target_network
        optimizer = optimizer_fc
        n_actions = fc_n_actions
        fc_loss_t = distributed_train_step_fc(batch_data)
