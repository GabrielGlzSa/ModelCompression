...
from CompressionLibrary.utils import calculate_model_weights, calculate_reward
...
agent_name = 'two_layers_custom_model'
...
if current_os == 'windows':
    ...
    fine_tuning_filename = data_path+f'/stats/{agent_name}_fine-tuning_stats.csv'
...
splits, info = tfds.load(datasets_name[0], as_supervised=True, with_info=True,
...
train_ds, valid_ds, test_ds = prepare_dataset(train_examples, validation_examples, test_examples, num_examples, map_fn,
                                              BATCH_SIZE) 
model = create_model(datasets_name[0], train_ds, valid_ds)
...
for compressor in compressors:
    ...
    temp_comp = class_(model=model, dataset=train_ds, optimizer=optimizer, loss=loss_object, metrics=train_metric, input_shape=input_shape,
                                fine_tuning=False)
    if temp_comp.target_layer_type == 'conv' and compressor in parameters:
        conv_compressors.append(compressor)
    elif temp_comp.target_layer_type == 'dense' and compressor in parameters:
...
for dataset in datasets_name:
    ...
    for conv_comp_name in conv_compressors:
        for dense_comp_name in dense_compressors:
            logging.info(f'Using compressors {conv_comp_name} and {dense_comp_name} for dataset {dataset}.')
            ...
            conv_class_ = getattr(CompressionTechniques, conv_comp_name)
            dense_class_ = getattr(CompressionTechniques, dense_comp_name)
            ...
            conv_compressor = conv_class_(model=model, dataset=train_ds, optimizer=optimizer, loss=loss_object, metrics=train_metric, input_shape=input_shape,
                                fine_tuning=False)
            
            
            if  not df.empty and df.loc[((df.dataset == dataset)  & (df.compressor_conv==conv_comp_name)&(df.compressor_dense==dense_comp_name))].shape[0]>=1:
                continue
            
            

            conv_compressor_params = parameters[conv_comp_name]
            conv_compressor.compress_layer(**conv_compressor_params)

            compressed_model = conv_compressor.get_model()

            dense_compressor = dense_class_(model=compressed_model, dataset=train_ds, optimizer=optimizer, loss=loss_object, metrics=train_metric, input_shape=input_shape,
                                fine_tuning=False)
            
            dense_compressor_params = parameters[dense_comp_name]
            dense_compressor.compress_layer(**dense_compressor_params)

            compressed_model = dense_compressor.get_model()

            
            # Calculate stats after compression.
            train_acc_after, val_acc_after, test_acc_after, weights_after = test_model(compressed_model, 'original')

            rbw = RestoreBestWeights(acc_before = val_acc_b, 
                                     reward_func=calculate_reward
                                     ,weights_before=weights_before,
                                     save_name=fine_tuning_filename,
                                     dataset_name=dataset, 
                                     verbose=1)
            if conv_compressor in ['SparseConnectionsCompression']:
            ...
            info['compressor_conv'] = conv_comp_name
            info['compressor_dense'] = dense_comp_name
            info['train_acc_before'] = train_acc_b
            info['train_acc_after_c'] = train_acc_after
            info['val_acc_before'] = val_acc_b
            info['val_acc_after_c'] = val_acc_after
            info['test_acc_before'] = test_acc_b
            info['test_acc_after_c'] = test_acc_after
            info['parameters_before'] = weights_before
            info['parameters_after_c'] = weights_after
            ...
            fine_tune_all_model.fit(train_ds, epochs=fine_tuning_epochs, validation_data=valid_ds, verbose=2, callbacks=cb)
            ...
            info['fine-tuning epochs'] = fine_tuning_epochs
            info['train_acc_after_f'] = train_acc_after
            info['val_acc_after_f'] = val_acc_after
            info['test_acc_after_f'] = test_acc_after
            info['parameters_after_f'] = weights_after
