...
from CompressionLibrary.reinforcement_models import DuelingDQNAgentROI as DuelingDQNAgent
...
dataset_names = ['fashion_mnist', 'kmnist'] #['fashion_mnist','kmnist']
agent_name = 'DDQN_roi_SVD_custom_model_sparse_rw_copy_50_train_1000_' + '-'.join(dataset_names)
...
verbose = 0
rl_iterations = 2000
update_weights_iterations = 1000
rl_batch_size = 128
gamma = 0.99
beta = 0.5
max_beta = 1.0
...
layer_name_list = ['conv2d_1', 'conv2d_2',  'dense_0', 'dense_1']

num_classes=10

def create_model(dataset_name, train_ds, valid_ds):
    if current_os =='windows':
        checkpoint_path = f"G:\\Python projects\\ModelCompressionRL\\data\\models\\custom_cnn_3conv_{dataset_name}\\cp.ckpt"
    else:
        checkpoint_path = data_path + f"/models/custom_cnn_3conv_{dataset_name}/cp.ckpt"
    ...
    for it in range(n_games):
        ...
        for k in range(1, len(env.original_layer_name_list)+1):
            ...
            if exploration:
                ...
                # new_s = temp
                if last_conv or done:
                    data.append([s, action, r, new_s, True, was_conv, last_conv])
                else:
                    data.append([s, action, r, new_s, done, was_conv, last_conv])
            ...
            if done:
                if exploration:
                    for row in data:
                        ...
                        s, a, rw, sn, done, conv, last_layer_type = row
                        ...
                        if conv:
                            ...
                            if last_layer_type:
                                ...
                                # change rw to r for dense rewards
                                td_errors = calculate_td_error_conv(s, actions_batch, [rw]*num_inst, sn, done_float)
