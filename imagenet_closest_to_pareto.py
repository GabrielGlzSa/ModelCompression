...
agent_name = 'closest2pareto' + '_' + dataset_name
...
df_pareto = pd.read_csv(data_path+'/stats/imagenet_pareto.csv')

for idx, row in df_pareto.iterrows():
    
    logger.debug(f'Processing solution {idx}')
    try:
        print(row['Sequence (%)'][1:-1].replace(' ',' ').split('.')[:-1])
        row_actions = list(map(lambda x: int(x), row['Sequence (%)'][1:-1].replace(' ',' ').split('.')[:-1]))
        print(row_actions)
        acc, weights_after = evaluation_function(row_actions)
    except:
        df_pareto.to_csv(data_path+'/stats/imagenet_pareto_processed.csv', index=False)
        
    df_pareto.at[idx, 'acc_so'] = acc
    df_pareto.at[idx, 'w_so'] = weights_after
