...
def update_agent_fc(state_batch, action_batch,reward_batch,next_state_batch,done,sample_probabilities):
    ...
    with tf.GradientTape() as tape:
        target_actions = fc_target_agent.actor(next_state_batch, training=True)
        critic_next_values = tf.math.multiply_no_nan(fc_target_agent.critic([next_state_batch, target_actions], training=True), (1.0 - done))
        y = reward_batch + gamma * critic_next_values
        critic_current_values = fc_agent.critic([state_batch, action_batch], training=True) 
    ...
    critic_grad = tape.gradient(critic_loss, fc_agent.critic.trainable_variables)
    fc_critic_optimizer.apply_gradients(zip(critic_grad, fc_agent.critic.trainable_variables))

    with tf.GradientTape() as tape:
        actions = fc_agent.actor(state_batch, training=True)
        critic_value = fc_agent.critic([state_batch, actions], training=True)
        # Used `-value` as we want to maximize the value given
        # by the critic for our actions
        actor_loss = -tf.math.reduce_mean(critic_value)

    actor_grad = tape.gradient(actor_loss, fc_agent.actor.trainable_variables)
    fc_actor_optimizer.apply_gradients(
        zip(actor_grad, fc_agent.actor.trainable_variables)
    )
    return td_error

@tf.function(experimental_relax_shapes=True)
def update_agent_conv(state_batch,
    action_batch,
    reward_batch,
    next_state_batch,
    done,
    ...
    with tf.GradientTape() as tape:
        ...
        critic_next_values = tf.math.multiply_no_nan(conv_target_agent.critic([next_state_batch, target_actions], training=True), (1.0 - done))
        y = reward_batch + gamma * critic_next_values
        critic_current_values = conv_agent.critic([state_batch, action_batch], training=True) 
        td_error = y - critic_current_values
        deltai = tf.math.square(td_error)
        importance_sampling = 1 / (replay_buffer_size** beta * sample_probabilities**beta)
        critic_loss = tf.math.reduce_mean(importance_sampling*deltai)
    ...
    conv_critic_optimizer.apply_gradients(zip(critic_grad, conv_agent.critic.trainable_variables))

    with tf.GradientTape() as tape:
        actions = conv_agent.actor(state_batch, training=True)
        critic_value = conv_agent.critic([state_batch, actions], training=False)
        # Used `-value` as we want to maximize the value given
        # by the critic for our actions
        actor_loss = -tf.math.reduce_mean(critic_value)

    actor_grad = tape.gradient(actor_loss, conv_agent.actor.trainable_variables)
    conv_actor_optimizer.apply_gradients(zip(actor_grad, conv_agent.actor.trainable_variables))
    return td_error
...
def calculate_td_error(agent, target_agent, state_batch, action_batch, reward_batch, next_state_batch, done):
    ...
    logger.debug(f'Target action is {tf.math.reduce_mean(target_actions)} for shape {next_state_batch.shape}')
    critic_next_value = tf.math.multiply_no_nan(target_agent.critic([next_state_batch, target_actions]),(1.0 - done))
    logger.debug(f'Critic value for next state is {tf.math.reduce_mean(critic_next_value)} for shape {critic_next_value.shape}')
    y = reward_batch + gamma * critic_next_value 
    logger.debug(f'Y is {tf.math.reduce_mean(y)} for shape {y.shape}')
    critic_current_value = agent.critic([state_batch, action_batch])
    logger.debug(f'Critic value for current state is {tf.math.reduce_mean(critic_current_value)} for shape {critic_current_value.shape}')
    ...
    for it in range(n_games):
        ...
        for k in range(1, len(env.layer_name_list)+1):
            ...
            if isinstance(layer, tf.keras.layers.Dense):
                was_conv = False
                action = fc_agent.policy(s, exploration=exploration)
            ...
            if exploration:
                if not skip_add_replay:
                    ...
                    if was_conv:
                        ...
                        td_errors = calculate_td_error(fc_agent, fc_target_agent, s, actions_batch, rewards_batch, new_s, done_float)
                        td_errors = np.reshape(np.abs(td_errors), -1)
...
with tqdm(total=rl_iterations,
    ...
    for i in range(rl_iterations):
        ...
        for w_it in range(update_weights_iterations):
            ...
            update_target(fc_target_agent.actor.variables, fc_agent.actor.variables, tau)
            update_target(fc_target_agent.critic.variables, fc_agent.critic.variables, tau)
